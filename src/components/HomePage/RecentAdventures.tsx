    ...
    return (
        <section className="bg-white text-black py-14 xl:px-10">
            <div className="relative w-full flex justify-center items-center">
                <h2 className="text-4xl font-serif inline-block text-center z-[2] bg-white px-8">My recent adventures</h2>
                <div className="max-w-6xl mx-auto w-full absolute top-[50%] hidden xl:block">
                    <div className="absolute top-0 left-0 bg-gradient-to-r from-transparent to-green-500/75 w-[50%] h-[2px] z-[1]"></div>
                    <div className="absolute top-0 right-0 bg-gradient-to-l from-transparent to-green-500/75 w-[50%] h-[2px] z-[1]"></div>
                </div>
            </div>
            <div className="flex flex-col xl:flex-row w-full max-w-6xl mx-auto mt-12 gap-0">
                {data.map((element, index) => (
                    <div key={index} className={`flex-1 flex flex-col items-center ${index != data.length - 1 ? "mb-8": "mb-0" } xl:mb-0`}>
                        <Link href={element["href"]} key={index}>
                            <div className="w-96 h-96 xl:w-80 xl:h-80 relative group overflow-hidden">
                                <Image
                                    src={`/${element["img"]}`}
                            ...
                            <div className="mt-4 text-center font-serif max-w-60 mx-auto">
